...
- **网页存档**：下载原文 HTML 快照并用 trafilatura 提取正文；存档前做**新闻页体检**（排除栏目页/专题页/聚合汇总贴/正文过短/纯列表页），提取失败的页面保留快照并标注
- **日期与周过滤**：从列表页、URL、页面元数据与正文多策略抽取发布日期；**只保留目标周（周一~周日）内发布的新闻**，日期缺失由模型兜底，仍无法确定则丢弃 —— 保证周报时效性
- **AI 打标**：逐条输出相关性判定（无关内容自动过滤）、标签（限定在标签树内，避免模型自造）、摘要、关键词、厂商/集成商公司名、重要度 1-3，其中厂商动态类设有重要度下限以保障占比；**默认按 10 条一批调用**（一次请求同时完成是否新闻、发布日期、标签、摘要、厂商等多项判断）
...
定时（Windows 任务计划，每周一 08:30）或手动执行
        ↓
① 采集  多源拉取候选条目 → 与历史记录比对去重（列表层顺带解析日期）
        ↓
② 存档  并发下载 HTML 快照 → 提取正文 → 新闻页体检 + 目标周过滤 → 落盘 + SQLite 索引
        ↓
③ 分类  大模型按批打标（每批 10 条，一次完成是否新闻/发布日期/标签/摘要/厂商/相关性）
        ↓
④ 综述  主题取自打标一级标签 → 一次大模型调用产出逐条要点 + 概览/TOP5/趋势/下周关注
        ↓
⑤ 产物  data/reports/<周>/weekly-<周>.{html,docx,json}
```

一次完整运行的 LLM 调用量约为「新闻条数 ÷ 10 + 1」次（百余条新闻约 10 次左右）。
...
**`config.yaml`**：`llm.provider` 与 `llm.model`（DeepSeek 官方模型名为全小写，如 `deepseek-flash`、`deepseek-v4-pro`）、`collect.*`（并发数、超时、采集节流）、`classify.*`（批次大小、每批正文截断、厂商重要度下限）、`archive.*`（周过滤开关、正文长度下限、链接密度上限、汇总贴过滤）、`report.*`（概览字数、要点条数、输入条数上限）、`notify.*`（异常告警与周报推送开关）。
