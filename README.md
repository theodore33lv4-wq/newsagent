...
```
[Windows 任务计划] 每周一 08:00（或手动）｜口径：生成【上一完整周】周报（今天-7天的ISO周，一周内任何一天运行都稳定落在同一周）
        ↓
...
新增源时把条目加入 `sources.yaml` 并置 `enabled: true` 即可；特殊站点（SPA/公众号/需登录）若静态采集不可行，请在日志确认后联系维护者评估适配方案。实测记录（截至 2026-09）：ITS114 官网（SSL 阻断）、中国政府网政策库（403）、公安部 122/mps（无 SSR）、中国政府采购网（反爬）暂不可用；赛文交通网 7its 改版后列表页 SSR 可用（已接入）；盖世汽车频道页为 JS 渲染但官方 RSS 可用（已接入）。
...
- **厂商动态占比**：2026-09 已接入赛文交通网（集成商资讯密集）后厂商动态开始产出；仍欢迎补充更多"集成商/中标/厂商"类源（见 `sources.yaml` 顶部探测记录）
- **微信生态**：公众号正文转存需 mp.weixin 直链（架构已兼容），暂无周期性渠道；搜狗微信搜索仅可做标题监测
